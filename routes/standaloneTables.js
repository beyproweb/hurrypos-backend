...
// GET /api/standalone/tables — list table configs
...
// PUT /api/standalone/tables/count { total }
...
// GET /api/standalone/tables/:number/qr-token
router.get("/:number/qr-token", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { type: "table", restaurant_id: restaurantId, table_number: number },
      TABLE_QR_SECRET,
      { expiresIn: "7d" }
    );
    ...
    const identifier =
      restaurant.slug || restaurant.qr_code_id || String(restaurantId);
    ...
    const url = `${cleanBase}/qr?mode=table&table=${encodeURIComponent(
      number
    )}&token=${encodeURIComponent(token)}&identifier=${encodeURIComponent(
      identifier
    )}`;
