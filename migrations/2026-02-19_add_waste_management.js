/**
 * Migration: Waste management tables
 * - stock_batches (per-delivery quantities with expiry + cost)
 * - stock_movements (auditable stock events, incl. waste & corrections)
 *
 * Safe to run multiple times (IF NOT EXISTS on tables/columns/indexes).
 */
...
async function migrateWaste() {
  ...
  try {
    console.log("🚀 Creating waste management tables (if missing)...");
    ...
    console.log("✅ Waste management tables ready.");
  } catch (err) {
    await client.query("ROLLBACK");
    console.error("❌ Waste migration failed:", err.message);
    throw err;
  } finally {
    client.release();
  }
