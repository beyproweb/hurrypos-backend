...
router.get('/roles', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT DISTINCT role FROM staff WHERE restaurant_id = $1 ORDER BY role');
...
router.post('/checkin', async (req, res) => {
  ...
  try {
    ...
    const staffCheck = await pool.query('SELECT id FROM staff WHERE restaurant_id = $1 WHERE id = $1', [staffId]);
...
router.post('/', async (req, res) => {
  ...
  try {
    const existingStaff = await pool.query('SELECT * FROM staff WHERE restaurant_id = $1 WHERE id = $1', [id]);
...
router.get('/:staffId/payroll', async (req, res) => {
  ...
  try {
    ...
    const staffRes = await pool.query(
      `SELECT salary, hourly_rate, salary_model, payment_type, weekly_salary, monthly_salary FROM staff WHERE restaurant_id = $1 WHERE id = $1`,
      [staffId]
    );
...
router.post('/:staffId/payments', async (req, res) => {
  ...
  try {
    ...
    const staffRes = await pool.query(
      `SELECT name, email, role FROM staff WHERE restaurant_id = $1 WHERE id = $1`,
      [staffId]
    );
...
router.get('/drivers', async (req, res) => {
  try {
    const result = await pool.query(
  `SELECT id, name, phone FROM staff WHERE restaurant_id = $1 WHERE LOWER(role) = 'kurye' OR LOWER(role) = 'driver'`
);
    res.json(result.rows);
  } catch (err) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    // 2. Try STAFF
    const staffResult = await pool.query("SELECT * FROM staff WHERE restaurant_id = $1 WHERE email = $1", [email]);
