...
async function buildFullOrderPayload(orderId) {
  const { rows: orderRows } = await pool.query(
    `SELECT id, status, table_number, order_type, total, created_at
     FROM orders WHERE restaurant_id = $1 WHERE id = $1`,
    [orderId]
  );
...
router.get("/", async (req, res) => {
  try {
    ...
    const sql = `
      ...
      FROM orders WHERE restaurant_id = $1 o
...
router.post("/order-items", async (req, res) => {
  ...
  try {
    ...
    await updateStockForOrder(preparedItems);
     // --- ADD THIS BLOCK:
    const orderRes = await pool.query("SELECT status FROM orders WHERE restaurant_id = $1 WHERE id = $1", [order_id]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (payment_method !== undefined) {
      const oldOrder = await pool.query('SELECT payment_method FROM orders WHERE restaurant_id = $1 WHERE id = $1', [id]);
  ...
  for (const item of orderItems) {
    ...
    for (const ing of ingredients) {
      ...
      const stockRes = await pool.query(
        `SELECT id, unit FROM stock WHERE restaurant_id = $1 WHERE LOWER(name) = LOWER($1) LIMIT 1`,
...
router.patch("/:id/reopen", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("SELECT * FROM orders WHERE restaurant_id = $1 WHERE id = $1", [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Order not found" });
    }
...
router.patch("/:id/driver-status", async (req, res) => {
  ...
  try {
    ...
    const driverCheck = await client.query(
      `SELECT driver_id FROM orders WHERE restaurant_id = $1 WHERE id = $1`,
      [id]
    );
...
router.patch("/:id/move-table", async (req, res) => {
  ...
  try {
    ...
    const orderRes = await client.query(
      `SELECT table_number FROM orders WHERE restaurant_id = $1 WHERE id = $1`,
      [id]
    );
...
router.patch("/:id/merge-table", async (req, res) => {
  ...
  try {
    ...
    const targetOrderRes = await client.query(
      `SELECT * FROM orders WHERE restaurant_id = $1 WHERE table_number = $1 AND status != 'closed' ORDER BY created_at DESC LIMIT 1`,
      [target_table_number]
    );
...
console.log("✅ confirm-online route loaded");
  ...
  try {
    ...
    // 1. Get order
    const { rows } = await client.query("SELECT * FROM orders WHERE restaurant_id = $1 WHERE id = $1", [id]);
    const order = rows[0];
    if (!order) {
      await client.query("ROLLBACK");
      return res.status(404).json({ error: "Order not found" });
