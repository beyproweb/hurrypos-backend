  ...
  try {
    await pool.query(`
      DELETE FROM order_items WHERE order_id IN (
        SELECT id FROM orders WHERE restaurant_id = $1 WHERE status = 'paid' OR status = 'closed'
      );
    `);
    await pool.query(`DELETE FROM sub_orders;`);
    await pool.query(`DELETE FROM orders WHERE restaurant_id = $1 WHERE status = 'paid' OR status = 'closed';`);
