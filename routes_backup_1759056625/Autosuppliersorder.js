...
router.post("/supplier-cart-items", async (req, res) => {
  try {
    ...
    const stockRes = await pool.query(
      `SELECT quantity, critical_quantity FROM stock WHERE restaurant_id = $1 WHERE id = $1`,
      [stock_id]
    );
...
router.post("/supplier-carts/:id/send", async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      const stockRes = await pool.query(
        `SELECT quantity, name FROM stock WHERE restaurant_id = $1 WHERE id = $1`,
        [item.stock_id]
      );
...
router.patch("/stock/:id", async (req, res) => {
  try {
    ...
    const currentRes = await pool.query(`SELECT * FROM stock WHERE restaurant_id = $1 WHERE id = $1`, [id]);
    const current = currentRes.rows[0];
...
router.get("/stock/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query("SELECT * FROM stock WHERE restaurant_id = $1 WHERE id = $1", [id]);
