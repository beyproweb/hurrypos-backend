  ...
  try {
    const result = await pool.query(`
      ...
      payment_losses AS (
        ...
        FROM transactions WHERE restaurant_id = $1 t
  ...
  try {
    ...
    if (type === "close") {
      const openOrdersRes = await pool.query(`
        SELECT COUNT(*) FROM orders WHERE restaurant_id = $1 WHERE status != 'closed'
      `);
...
router.post("/receipt-methods", async (req, res) => {
  ...
  try {
    ...
    const { rows: orderRows } = await pool.query(
      `SELECT id, payment_method FROM orders WHERE restaurant_id = $1 WHERE receipt_id = $1`,
      [receipt_id]
    );
